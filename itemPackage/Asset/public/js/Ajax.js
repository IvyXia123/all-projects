...
makeLoans.scrollSize = function(dom){
	dom.find('table').each(function(){
		var tbodyW = $(this).children('tbody').outerWidth()-16;
		// u-tab 增加滚动条样式
		if($(this).parents('.u-tab').width() >= tbodyW){
			$(this).parents('.u-tab').css({'overflow-x':'hidden'});
			return
		} else {
			var bodyW = $(this).children('tbody').outerWidth();
			$(this).parents('.u-tab').css({'overflow-x':'scroll'})
					.children().css({'overflow':'hidden','width':bodyW})
					.children('.head-tab').css('width',$(this).parents('.u-tab').children().width());
			$(this).parent('.body-tab').css('width',bodyW+12);
		}
	})
}
