...
function querySubmit(event){
	...
	if( flex.attr('id') == 'laterPeriodCosts-m-flex' && Collect.replace(/\s+/g,"").length == 10){
        ...
        * */
        var $tbody = $('#makeLoans-m-flex').find('#ta').find('tbody');
		$tbody.empty();
        ...
        for (var i = 0; i < useObj.length; i++) {
        	// 查询信息
	       	...
	       	// 资产放款日
	       		if(InvestmentLendingDayStart == undefined)return;
	        	if(InvestmentLendingDayStart <= useObj[i].inception_date){
	        		if(InvestmentLendingDayEnd >= useObj[i].due_date || InvestmentLendingDayEnd == ""){
			        	var $tbodyTr = $('<tr></tr>');
		                // each thead,The tbody content and thead consistent sequence
		                for (var n = 0; n < Array.length; n++) {
		                    var field = Array[n].field;
	                    	// 序号
		                    if (field == 'loan_id') {
		                        var td = '<td>' + (count+1) + '</td>';
		                        // 投资收益合计;
		                    } else if (field == 'profit') {
		                        profit += Number(useObj[i][field]);
		                        // 投资金额合计
		                    } else if (field == 'amount') {
		                        amount += Number(useObj[i][field]);
		                        // 保留三位
		                    } else if (field == 'marketing_extra_yield_rate' || field == 'real_rate') {
		                        var nub = parseFloat(useObj[i][field]).toFixed(3);
		                        var td = '<td class="pointer flexTab-create">' + nub + '</td>';
		                        // 资产编号特殊设置，用于操作下级tab;
		                    } else {
		                    	if(useObj[i][field] == undefined){
		                        	var td = '<td></td>';
		                    	}else{
		                        	var td = '<td>' + useObj[i][field] + '</td>';
		                    	}
		                    }
		                    //金额显示特殊处理
		                    if (!isNaN(Number(useObj[i][field]))) {
		                        if ($.trim(useObj[i][field]).length < 1 || field == 'loan_bank_no' || field == 'repay_bank_no'
		                                || field == 'rate_days' || field == 'loan_id' || field == 'month_category'
		                                || field == 'product_limit' || field == 'per_year_date' || field == 'product_limit'
		                                || field == 'asset_manager_bank_acct' || field == 'prod_issuer_bank_acct'
		                                || field == 'marketing_extra_yield_rate' || field == 'real_rate'
		                            ...
		                            var t = parseFloat(useObj[i][field]).toFixed(2);
		                            if (t.indexOf('.00') >= 0) {
        ...
        // tab style
        Pages.scrollSize($('#flexTab-u-flex4'));
        queryBox.hide();
...
function sellingTime(e){
    ...
    var endTime   = ($(selectSearchList[1]).val()).replace(/\s+/g,"");
    	...
    	if(endTime.length == 12 && $(selectSearchList[1]).val().length > 13){
	        ...
	        var details = {
	            Url: 'http://ds.idc.xiwanglife.com/dataservice/getconfig.do?id=88&start_time='+searchVal[0]+'&end_time='+searchVal[1],
	            Array: flex.arr[2],
